...
ggml_tensor * llm_graph_context::build_attn(
    ...
    if (auto * no_cache = dynamic_cast<llm_graph_input_attn_no_cache*>(inp)) {
            return build_attn(no_cache, wo, wo_b, q_cur, k_cur, v_cur, kq_b, sinks, v_mla, kq_scale, il);
    }
    if (auto * kv = dynamic_cast<llm_graph_input_attn_kv*>(inp)) {
        return build_attn(kv, wo, wo_b, q_cur, k_cur, v_cur, kq_b, sinks, v_mla, kq_scale, il);
    }
    if (auto * iswa = dynamic_cast<llm_graph_input_attn_kv_iswa*>(inp)) {
            return build_attn(iswa, wo, wo_b, q_cur, k_cur, v_cur, kq_b, sinks, v_mla, kq_scale, il);
    }
    if (auto * paged = dynamic_cast<llm_graph_input_attn_kv_paged*>(inp)) {
        return build_attn(paged, wo, wo_b, q_cur, k_cur, v_cur, kq_b, sinks, v_mla, kq_scale, il);
    }
